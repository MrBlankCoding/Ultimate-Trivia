...
def dbl_webhook():
    try:
        ...
        now = datetime.utcnow()  # Use UTC format for MongoDB
        asyncio.run_coroutine_threadsafe(
            asyncio.to_thread(
                bot.db.upvotes.update_one,  # Run the blocking call in a separate thread
                {'user_id': user_id},
                {'$set': {'last_upvote': now}},
                upsert=True
            ), 
        ...
        try:
            result = future.result()  # Handle the future result
        except Exception as e:
            app.logger.error(f"Error awaiting future result: {str(e)}")
